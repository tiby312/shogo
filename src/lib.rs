...
pub async fn start() -> Result<(), JsValue> {
    ...
    let (mut socket, _wsio) = socket_create
        .await
        .expect("websocket socket creation failed");
    let (mut socket2, _wsio) = socket2_create
        .await
        .expect("websocket socket creation failed");
    ...
    let audio_buffer = audio_ctx
        .create_buffer(
            1,
            (audio_ctx.sample_rate() * 2.0) as u32,
...
pub fn on_animation_frame(timestamp: f64) {
    ...
    let left_direction = if left.up == left.down {
        0.
    } else if left.up {
        1.
    } else {
    ...
    let right_direction = if right.up == right.down {
        0.
    } else if right.up {
        1.
    } else {
    ...
    left.position.y = clamp(
        left.position.y + left_direction * PADDLE_SPEED * delta,
        -0.8,
        0.8,
    );
    right.position.y = clamp(
        right.position.y + right_direction * PADDLE_SPEED * delta,
        -0.8,
        0.8,
    );
    ...
    if collide(
        &ball.position,
        &pong.ball_model.extent,
        &left.position,
        &pong.paddle_model.extent,
    ) || collide(
        &ball.position,
        &pong.ball_model.extent,
        &right.position,
        ...
        create_sparks(
            &mut pong.sparks,
            (if ball.velocity.x > 0. { -1. } else { 1. }) * pong.ball_model.extent.x
                + ball.position.x,
            ball.position.y,
            2. * ball.velocity.x,
            0.,
    ...
    if collide(
        ...
        create_sparks(
            &mut pong.sparks,
            ball.position.x,
            (if ball.velocity.y > 0. { -1. } else { 1. }) * pong.ball_model.extent.y
                + ball.position.y,
            0.,
            2. * ball.velocity.y,
        );
    ...
    pong.ball_tail
        .add(ball.position.clone(), Vec2::zero(), Vec2::zero(), 1000);
    ...
    pong.ctx
        .gl
        .enable_vertex_attrib_array(pong.ctx.position as u32);
    pong.ctx
        .gl
        .enable_vertex_attrib_array(pong.ctx.texcoord as u32);
...
impl Model {
    ...
    fn pre_render(&self, ctx: &RenderContext) {
        ...
        ctx.gl
            .vertex_attrib_pointer_with_i32(ctx.position as u32, 2, GL::FLOAT, false, 16, 0);
        ctx.gl
            .vertex_attrib_pointer_with_i32(ctx.texcoord as u32, 2, GL::FLOAT, false, 16, 8);
    ...
    fn render(&self, pos: &Vec2, ctx: &RenderContext) {
        ...
        ctx.gl
            .draw_arrays(GL::TRIANGLES, 0, self.num_vertices as i32 / 4);
    }
...
fn set_score(left: u32, right: u32) {
    ...
    document
        .get_element_by_id("score_left")
        .unwrap()
        .set_text_content(Some(&left.to_string()));
    document
        .get_element_by_id("score_right")
        .unwrap()
